...
public class UserServiceImpl implements IUserService {
	...
	public APIResponse updateUserDetailsById(UserDetailsDTO userDetailsDTO, Integer id) {
		...
		if(updateUser == null) {
			...
			if(userDetailsDTO.getFirstName() != null && userDetailsDTO.getFirstName().length()>0) {
				updateUser.setFirstName(userDetailsDTO.getFirstName());
			}
			if(userDetailsDTO.getLastName() != null && userDetailsDTO.getLastName().length()>0) {
				updateUser.setLastName(userDetailsDTO.getLastName());
			}
			if(userDetailsDTO.getAddress() != null && userDetailsDTO.getAddress().length()>0) {
